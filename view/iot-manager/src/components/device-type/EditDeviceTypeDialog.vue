<script setup>
    ...
    const showModal = async id => {
        const r = await http.get(`/user/device-type/id/${id}`)
    ...
    // 点击确认按钮提交
    const editDeviceType = async () => {
        try {
            await formRef.value.validateFields() 
            try {
                const r = await http.post(`/user/device-type/update`, editForm.value)
                if(r.code === 200) {
    ...
    <a-modal
        ...
        >
            ...
            <a-form-item label="设备所属行业" name="industryId">
                <a-select
                    v-model:value="editForm.industryId"
                    style="width: 100%"
            ...
            <a-form-item label="设备类型" name="deviceType">
                <a-select
                    v-model:value="editForm.deviceType"
                    style="width: 100%"
