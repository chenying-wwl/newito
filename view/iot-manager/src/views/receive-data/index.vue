<script setup>
    ...
    const {data, columns, searchForm, total, flip, search, clear, formRef } = useReceiveData(props.id)
...
<template>
    <a-breadcrumb style="margin: 16px 0">
        ...
        <a-breadcrumb-item>设备接收数据</a-breadcrumb-item>
    </a-breadcrumb>
    <a-table size="small" style="margin-top: 1em;" :columns="columns" :data-source="data" :pagination="false">
        <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'tenant_id'">
                {{ filterTenant(record.tenant_id) }}
            </template>
            <template v-if="column.key === 'product_key'">
                {{ filterProduct(record.product_key) }}
            </template>
            <template v-if="column.key === 'device'">
                {{ filterDevice(record.device) }}
            </template>
        </template>
    </a-table>
    <a-pagination 
