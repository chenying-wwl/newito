<script setup>
    ...
    const {data, columns, searchForm, total, flip, search, 
        clear, formRef, yesOrNO } = useTenant('examine')
    ...
    <a-table size="small" style="margin-top: 1em;" :columns="columns" :data-source="data" :pagination="false">
        <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
                <a-button size="small" type="link" @click="yesOrNO(record.id, 1)" class="action-btn">通过</a-button>
                <a-button size="small" type="link" @click="yesOrNO(record.id, 0)" class="action-btn">不通过</a-button>
            </template>
        </template>
    </a-table>
    <a-pagination 
